...
export default function ItineraryConfirmationPage() {
  ...
  const handleProceedWithBooking = async () => {
    ...
    try {
      // Call backend API to trigger booking
      const response = await fetch('/api/book-flights', {
        method: 'POST',
        headers: {
      ...
      // Start polling for flight_booking URL
      ...
      while (elapsed < pollTimeout) {
        ...
        if (groupError) {
          throw new Error('Failed to poll for booking link');
        }

        if (groupData && groupData.flight_booking && typeof groupData.flight_booking === 'string' && groupData.flight_booking.startsWith('http')) {
          // Open the booking link in a new tab
          window.open(groupData.flight_booking, '_blank');
          found = true;
          break;
        }
      }
      ...
      if (!found) {
        alert('Booking link was not generated in time. Please try again later.');
      }
    } catch (error) {
